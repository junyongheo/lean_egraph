...
def rewrite_search {α : Type _} {D : Type _} [DecidableEq α] [Hashable α] [analysis : Analysis α D] [Inhabited D] [Inhabited (Pattern α)] (r : Rule α D) : EGraphM α D <| List (Rule α D × EClassId × Dict α) := do
  let eg ← get
